...
wincore.cef = {
    ...
    aSyncSendMessage: function (that, method_name, tag, msg, nolog = false) {
        try {
            if (cef != undefined) {
                var method_id = wincore.cef.get_method_id(that, method_name, 'async');
                cef.message.sendMessage(method_id, [msg, tag]);
                !nolog && console.log("aSyncSendMessage, " + method_id + ", " + msg);
            }
        } catch (e) {
...
(function (global) {
    ...
    wincore.oo.System = wincore.util.createClass(wincore.Observable, {
        ...
        // 获取当前窗口缩放到1920所需要设置的zoomlevel
        get_zoom1920level: function () {
            var result = wincore.cef.syncSendMessage(this, 'get_zoom1920level', '', JSON.stringify({ uuid: this.uuid, name: this.name }));
        ...
        shell_execute: function (exePath, params, cmd, shut_redirect, show) {
        ...
        get_stupc_register: function () {
            var result = wincore.cef.syncSendMessage(this, 'get_stupc_register', '', JSON.stringify({ uuid: this.uuid, name: this.name }));
            return wincore.JSONParse(result).data;
        },
        register_stupc: function (auth_code) {
            var result = wincore.cef.syncSendMessage(this, 'register_stupc', '', JSON.stringify({ uuid: this.uuid, name: this.name, auth_code: auth_code }));
            return wincore.JSONParse(result).data;
        },
        get_unique_id: function () {
            var result = wincore.cef.syncSendMessage(this, 'get_unique_id', '', JSON.stringify({ uuid: this.uuid, name: this.name }));
            return wincore.JSONParse(result).data.result;
        }
    });
...
(function (global) {
    ...
    wincore.oo.OSSUpload = wincore.util.createClass(wincore.Observable, {
        ...
        initialize: function (data) {
            ...
            var result = wincore.cef.syncSendMessage(this, 'create', '',
                JSON.stringify({
                    name: data.name,
...
(function (global) {
    ...
    wincore.OBSUpload = {};
    wincore.oo.OBSUpload = wincore.util.createClass(wincore.Observable, {
        module_name: 'OBSUpload',
        name: '',
        uuid: '',
        initialize: function (data) {
            data.name = data.name || ('OBSUpload' + wincore_oo_index++);
            var result = wincore.cef.syncSendMessage(this, 'create', '',
                JSON.stringify({
                    name: data.name,
                    filePath: data.filePath,
            ...
            this.uuid = obj.data.uuid;
            this.name = data.name;
            wincore.OBSUpload[this.name] = this;
        },
        release: function () {
            if (this.uuid) {
                wincore.cef.syncSendMessage(this, 'release', '', JSON.stringify({ uuid: this.uuid, name: this.name }));
                delete wincore.OBSUpload[this.name];
                this.uuid = this.name = null;
            }
        },
        on_progress: function (options) {
            this.trigger("on_progress", options);
            console.log("OBSUpload, on_progress: ");
        },
        on_complete: function (options) {
            this.trigger("on_complete", options);
            console.log("OBSUpload, on_complete: ");
            this.release();
        },
        on_failed: function (options) {
            this.trigger("on_failed", options);
            console.log("OBSUpload, on_failed: ");
            this.release();
        }
    });
})(typeof exports !== 'undefined' ? exports : this);
...
(function (global) {
    ...
    wincore.oo.Disk = wincore.util.createClass(wincore.Observable, {
        ...
        global_get_allvalue: function () {
            var data = { uuid: this.uuid, name: this.name };
            var result = wincore.cef.syncSendMessage(this, 'global_get_allvalue', '', JSON.stringify(data));
            var val = wincore.JSONParse(result).data.value;
            var valArray = decodeURIComponent(val).split('*,*');
            var outJSON = {};

            valArray.forEach(function (item) {
                let _arr = item.split('*=*');
                outJSON[_arr[0]] = _arr[1]
            })
            console.table(outJSON);
            return outJSON;
        },
        ...
        write_file: function (file_name, data, format, mode, callback, nolog = false) {
            var data = { uuid: this.uuid, name: this.name, file_name: file_name, data, mode };
            var event_name = '' + wincore_event_index;
            if (format) data['format'] = format;
            wincore.cef.aSyncSendMessage(this, 'write_file', event_name, JSON.stringify(data), nolog);
            if (callback) {
         ...
         * 锁住文件
         **/
        sync_lock_file(file_name, callback) {
            var data = { uuid: this.uuid, name: this.name, file_name: file_name };
            wincore.cef.aSyncSendMessage(this, 'lock_file', '', JSON.stringify(data));
            if (callback) {
                this.on("callback__lock_file", callback);
            }
        },
         ...
         * 锁住文件夹
         **/
        sync_lock_folder(src, callback) {
            var data = { uuid: this.uuid, name: this.name, path_src: src };
            wincore.cef.aSyncSendMessage(this, 'lock_folder', '', JSON.stringify(data));
            if (callback) {
                this.on("callback__lock_folder", callback);
            }
        },
        ...
        delete_files: function (file_list, callback) {
            var data = { uuid: this.uuid, name: this.name, file_list: file_list.join(";") };
            wincore.cef.aSyncSendMessage(this, 'delete_files', '', JSON.stringify(data));
            if (callback) {
                this.on("callback__delete_files", callback);
            }
        },
        ...
        copy_folder: function (src, dest, callback) {
            var data = { uuid: this.uuid, name: this.name, path_src: src, path_dest: dest };
            var event_name = '' + wincore_event_index;
            wincore.cef.aSyncSendMessage(this, 'copy_folder', event_name, JSON.stringify(data));
            if (callback) {
                this.on("callback__copy_folder_" + event_name, callback);
            }
            wincore_event_index++;
        },
        rename_folder: function (src, dest, callback) {
            var data = { uuid: this.uuid, name: this.name, path_src: src, path_dest: dest };
            var event_name = '' + wincore_event_index;
            wincore.cef.aSyncSendMessage(this, 'rename_folder', event_name, JSON.stringify(data));
            if (callback) {
                this.on("callback__rename_folder_" + event_name, callback);
            }
            wincore_event_index++;
        },
        move_folder: function (src, dest, callback) {
            var data = { uuid: this.uuid, name: this.name, path_src: src, path_dest: dest };
            var event_name = '' + wincore_event_index;
            wincore.cef.aSyncSendMessage(this, 'move_folder', event_name, JSON.stringify(data));
            if (callback) {
                this.on("callback__move_folder_" + event_name, callback);
            }
            wincore_event_index++;
        },
        ...
        writetext_clipboard: function (data, callback) {
            var data = { uuid: this.uuid, name: this.name, data: data };
            var event_name = '' + wincore_event_index;
            wincore.cef.aSyncSendMessage(this, 'writetext_clipboard', event_name, JSON.stringify(data));
            if (callback) {
                this.on("callback__writetext_clipboard_" + event_name, callback);
            }
            wincore_event_index++;
        },
        readtext_clipboard: function (callback) {
            var data = { uuid: this.uuid, name: this.name, data: "" };
            var event_name = '' + wincore_event_index;
            wincore.cef.aSyncSendMessage(this, 'readtext_clipboard', event_name, JSON.stringify(data));
            if (callback) {
                this.on("callback__readtext_clipboard_" + event_name, callback);
            }
            wincore_event_index++;
        },
        ...
        make_dir: function (dirpath, callback) {
            ...
            wincore.cef.aSyncSendMessage(this, 'make_dir', '', JSON.stringify(data));
            if (callback) {
                this.on("callback__make_dir", callback);
            }
        },
        remove_dir_allfile: function (dirpath, callback) {
            var data = { uuid: this.uuid, name: this.name, path: dirpath };
            wincore.cef.aSyncSendMessage(this, 'remove_dir_allfile', '', JSON.stringify(data));
            if (callback) {
                this.on("callback__remove_dir_allfile", callback);
            }
        },
        remove_dir: function (dirpath, callback) {
            var data = { uuid: this.uuid, name: this.name, path: dirpath };
            wincore.cef.aSyncSendMessage(this, 'remove_dir', '', JSON.stringify(data));
            if (callback) {
                this.on("callback__remove_dir", callback);
            }
        },
        ...
        // 添加文件到压缩包
        add_compress_file: function (srcfile, destfile, addpath, callback) {
            wincore.cef.aSyncSendMessage(this, 'add_compress_file', '', JSON.stringify({
                srcfile: srcfile,
                destfile: destfile,
                addpath: addpath
            }));
            if (callback) {
                this.on("callback__add_compress_file", callback);
            }
        },
        //不删除源文件，有需要手动调用删除方法
        ...
        // 获取华为AES加密
        get_base64_unicode_binary: function (text) {
            var data = { uuid: this.uuid, name: this.name, text: text };
            var result = wincore.cef.syncSendMessage(this, 'get_base64_unicode_binary', '', JSON.stringify(data));
            return wincore.JSONParse(result).data.value;
        },
        //获取桌面文件
        get_desktop_file: function () {
            var data = { uuid: this.uuid, name: this.name };
            var result = wincore.cef.syncSendMessage(this, 'get_desktop_file', '', JSON.stringify(data));
            return wincore.JSONParse(result).data;
        },
        // 打开系统选择文件夹窗口
        select_folder(title, default_folder, parentwnd_id) {
            var data = { uuid: this.uuid, name: this.name, title, default_folder, parentwnd_id };
            var result = wincore.cef.syncSendMessage(this, 'select_folder', '', JSON.stringify(data));
            return wincore.JSONParse(result).data;
        },
        /**
         * 打开系统选择文件窗口
         * @param {object} filter 过滤器{typeName:'图片文件', type:'*.png;*.jpeg'}
         * @param {string} parent 初始目录
         * @param {string} title  选择框标题
         * @param {bool}   multi  是否多选 false
         * @returns 
         * {
         *      ret: 1,
         *      files: []  
         * }
         */
        select_file(filter, parent, title, multi) {
            var type = ''
            var typeName = ''
            if (filter) {
                type = filter.type
            ...
            var result = wincore.cef.syncSendMessage(this, 'select_file', '', JSON.stringify(data));
            return wincore.JSONParse(result).data;
        },
    });
...
(function (global) {
    ...
    wincore.oo.BrowserForm = wincore.util.createClass(wincore.Observable, {
        ...
        initialize: function (data) {
            this.zoomTo1920 = this.zoomTo1920.bind(this);
            ...
            // 窗口创建成功，执行zoom_1920
            this.once('on_child_create', this.zoomTo1920)
            // 窗口从最小化恢复，重新执行执行zoom_1920
            this.on('on_size_change', evt => {
                if (evt.data.size === 'show' || evt.data.size === 'restore') {
                    this.zoomTo1920()
                }
            })
        ...
        // 锁定窗口缩放
        zoomTo1920() {
            if (this.zoomlevel !== 'zoom_1920') return;

            let zoom1920Level = wincore.System.get_zoom1920level().zoomlevel;
            this.set_page_zoom(zoom1920Level)
        },

        reload_uri: function (uri, callback) {
            if (!this.page_on_create) {
                // 检测页面完全loadend后再刷新页面
                this.once('on_create', () => this.reload_uri(uri, callback));
                return;
            }
        ...
        get_parent_info() {
            var result = wincore.cef.syncSendMessage(this, 'get_parent_info', '', JSON.stringify({ uuid: this.uuid, name: this.name }));
            return wincore.JSONParse(result).data;
        },
        get_pos: function (id) {
            var result = wincore.cef.syncSendMessage(this, 'get_pos', '', JSON.stringify({ uuid: this.uuid, name: this.name, id: id || '' }));
            return wincore.JSONParse(result).data;
        },
        ...
        set_restore: function (callback) { //还原
            wincore.cef.aSyncSendMessage(this, 'set_restore', '', JSON.stringify({
                uuid: this.uuid,
                name: this.name
            }));
            if (callback) {
                this.on("callback__set_restore", callback);
            }
        },
        /**
         * 设置缩放
         * @param {number} zoom 缩放值，范围1-5
         * 方法名和指令保持差异，防止回调重复进入
         **/
        set_page_zoom: function (zoom) {
            this.zoom = zoom;
            wincore.cef.aSyncSendMessage(this, 'set_zoom', '', JSON.stringify({ uuid: this.uuid, zoom }));
        },
        get_page_zoom: function () {
            return this.zoom;
        },
        ...
        on_create: function (options) {
            if (this.page_on_create) return;

            wincore.System.trigger(this.name + ".on_create", options);
            wincore.System.trigger("on_create", options);
        ...
        on_close: function (options) {
            ...
            this.release();
            wincore.System.trigger("on_release", options);
        },
...
(function (global) {
    ...
    wincore.oo.LivePlayer = wincore.util.createClass(wincore.Observable, {
        ...
        set_allowrfb: function (flag) {
            ...
            wincore.cef.aSyncSendMessage(this.live_session, 'set_allowrfb', '', JSON.stringify(param));
            console.log("LivePlayer set_allowrfb:" + flag);
        }
    });
...
(function (global) {
    ...
    wincore.oo.AsyncEvent = wincore.util.createClass(wincore.Observable, {
        ...
        // 添加文件到压缩包
        add_compress_file(src, dest, callback) {
            var event_name = '' + wincore_event_index;
            wincore.cef.aSyncSendMessage(this, 'add_compress_file', event_name, JSON.stringify({ uuid: this.uuid, name: this.name, id: event_name, src: src, dest: dest }));
            if (callback) {
                this.listen_once('on_event_fire.' + event_name, callback);
            }
            wincore_event_index++;
        ...
        async_action(action, src, dest, callback) {
            var event_name = '' + wincore_event_index;
            wincore.cef.aSyncSendMessage(this, action, event_name, JSON.stringify({ uuid: this.uuid, name: this.name, id: event_name, src, dest }));
            if (callback) {
                this.listen_once('on_event_fire.' + event_name, callback);
            }
            wincore_event_index++;
...
(function (global) {
    ...
    wincore.oo.FFmpegCmd = wincore.util.createClass(wincore.Observable, {
        ...
        download_video(file_path, output_path) {
            let result = this._asyncMsg('download_video', { file_path, output_path })
        },
        get_size(file_path) {
            let result = this._asyncMsg('get_size', { file_path })
        },
...
// 如果当前窗口在cef中，将wincore替换为父层
if (self !== top) {
    if (window.parent.wincore) {
        window.wincore = window.parent.wincore;
    } else {
        window.parent.wincore = window.wincore;
    }
}